...
plot_gage_timeseries <- function(gage_melt, yr, font_fam, break_value,
                                 y_expansion_value, text_smoothing_value){
  ...
  gages_by_year |>
    ...
    geom_textline(data = gages_by_year,
                  ...
                  hjust = 0.05,
                  color = 'grey70', size = 8/.pt, vjust = -0.35,
                  text_smoothing = text_smoothing_value,
    ...
    scale_y_continuous(
      breaks = scales::breaks_width(break_value),
      expand = expansion(add = c(y_expansion_value,0))
    ) +
    ylab(NULL) + xlab(NULL) +
    theme(
...
#' @param state_map shifted states and territories
plot_gage_map_state <- function(gage_melt, active_year, site_map, state_map,
                                county_map){
  ...
  sites.sf <- site_map |> st_as_sf() |>
    distinct() |>
    st_transform(st_crs(state_map)) |>
    st_intersection(state_map) # not sure this is needed
  ...
  gage_map <- gages_active |>
    ...
    geom_sf(data = state_map, fill = '#ededed',
            linewidth = 0.25, color = 'white') +
    geom_sf(data = county_map, fill = NA,
...
compose_chart <- function(bar_chart, gage_map, year, outfile_template){
  ...
  ggsave(sprintf(outfile_template, year), width = 5, height = 4, dpi = 300, units = 'in')
